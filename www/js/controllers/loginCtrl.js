angular.module('starterCtrl')
  .controller('loginCtrl', ["$scope", "$window", "$state", "$ionicHistory", "$rootScope", '$http','UsersREST',
    function($scope, $window, $state, $ionicHistory, $rootScope, $http, UsersREST) {
     ...
     $scope.type = type;   
      ...
      $scope.loginFormAnySession = function(form) {
        if (!form.$valid)
        
        return ("ERROR.TITLE", "ERROR.INCOMPLETE_FORM");
        var dateToSend = new Date();
        $scope.loginData.lastLog = dateToSend.toUTCString();
        
        
          UsersREST.login($scope.loginData)
          .then(function(response) {
            ...
            if (response.status === 200){
              ...
              if (response.data === null)
                return ("ERROR.TITLE", "ERROR.INTERNET_CONNECTION");
